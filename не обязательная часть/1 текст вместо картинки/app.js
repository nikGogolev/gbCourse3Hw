...
function handleClick(clickedButtonEvent) {
    ...
    const card = {
        ...
        button: event.target, // здесь button, который внутри .product
    };

    // 4. получаем текст на кнопке, которая внутри .product
    let buttonText = event.target.textContent;
    
    if (event.target.textContent === 'Подробнее') { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card);
    } else if (event.target.textContent === 'Отмена') { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function showMoreText(card) {
    ...
    let randomText = 'над этой кнопкой появлялся какой-нибудь текст'
